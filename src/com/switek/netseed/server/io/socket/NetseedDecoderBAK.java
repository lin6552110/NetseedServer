...
public class NetseedDecoderBAK implements ProtocolDecoder {

	Hashtable<Long, ArrayList<Byte>> packetHashtable = new Hashtable<>();
...
//				+ Thread.currentThread().getId() + session);
		...
		ArrayList<Byte> packetBytes = packetHashtable.get(sessionId);
		if (packetBytes==null){
		...
		if (packetBytes.size() < 15) {
			ServerForm.showLog(MsgType.Warn, "Invalid packet len. "
