...
import com.switek.netseed.server.ScheduleJobEngine;
...
public class ServerForm {
	...
	public void open() {
		...
		stopSocketServer();
		ScheduleJobEngine.newInstance().stop();
	...
	protected void loadList() {
		try {
			...
			DeviceManager.loadTimers();
			showCountInfo();
						
			startScheduleJobEngine();
		} catch (Exception ex) {
			showLog(ex);
		}
	}
	
	private void startScheduleJobEngine() {
		ServerForm.debugMsg("Setup schedule job engine.");
		ScheduleJobEngine.newInstance().start();
	}
	...
	protected void createContents() {
		...
		hprlnkCtrlCount.addHyperlinkListener(new IHyperlinkListener() {
			@Override
			public void linkActivated(HyperlinkEvent arg0) {
				ControllerStatusForm controllerStatusForm = new ControllerStatusForm();
				controllerStatusForm.open();
			}
			@Override
			public void linkEntered(HyperlinkEvent arg0) {
				//MessageBox.show("linkEntered");
			}
			@Override
			public void linkExited(HyperlinkEvent arg0) {
				//MessageBox.show("linkExited");
			}
		});
		...
		lbSystemRunTime.setText("0d 00:00:00");
			...
			public void run() {
				...
				display.asyncExec(new Runnable() {
					...
					public void run() {
						...
						String runTimeString = String.format("%sd %s:%s",
								runTimeDays,
								Utils.formatDecimal(runTimeHours, "00"),
								Utils.formatDecimal(runTimeMinutes, "00"));
						
						if(System.currentTimeMillis() - lastLogStartupTime >= 60 * 1000){
							lastLogStartupTime= System.currentTimeMillis();
							ServerForm.showLog("System run time: " + runTimeString);
						}
						lbSystemRunTime.setText(runTimeString);
						lbSystemRunTime.pack(true);
						shlCzIotServer.redraw();
