...
public class DeviceManager {
	...
	private static Hashtable<String, ControllerTimer> mTimers = new Hashtable<>();
	...
	public static void addTimer(ControllerTimer timer){
		getTimers().put(timer.getTimerId(), timer);		
	}

	public static Hashtable<String, ControllerTimer> getTimers() {
		return mTimers;
	}
	
	public static ControllerTimer getTimer(String Id){
		return mTimers.get(Id);
	}
	
	public static void deleteTimer(ControllerTimer timer){
		getTimers().remove(timer.getTimerId());
	}
	...
	public static void loadTimers(){
		...
		ResultSet ctrlSet = DB
				.executeQuery("select * from adtimer where isdeleted = 0;");

		Hashtable<String, ControllerTimer> tmpTimers = new Hashtable<>();
		try {
			while (ctrlSet.next()) {
				String Id = ctrlSet.getString("TimerId");
				String name = ctrlSet.getString("TimerName");
				int weekdays = ctrlSet.getInt("Weekdays");
				String time = ctrlSet.getString("TriggerTime");
				boolean enabled = ctrlSet.getBoolean("IsEnabled");
		...
		ServerForm.showLog(MsgType.Info, "There are " + tmpTimers.size()
				+ " timer(s)");

		ServerForm.showLog(MsgType.Info,
				"Loading timer steps...");
		ResultSet stepSet = DB
				.executeQuery("select * from adtimersteps as s join adtimer t on s.timerid = t.timerid and t.isenabled = 1 and t.isdeleted = 0");
		try {
			while (stepSet.next()) {
				String timerId = stepSet.getString("TimerId");
				int seqNo = stepSet.getInt("SeqNo");
				String stepName = stepSet.getString("StepName");
				String controllerId = stepSet.getString("ControllerId");
				String subId = stepSet.getString("SubControllerId");
				String deviceId = stepSet.getString("DeviceId");
				int keyIndex = stepSet.getInt("keyindex");
				int delay = stepSet.getInt("delay");
				int value = stepSet.getInt("value");
				ControllerTimer timer = tmpTimers.get(timerId);
				if (timer == null) {
					ServerForm.showLog(MsgType.Warn,
							"Couldn't find timer by Id: " + timerId);
					continue;
				}

				TimerStep step = new TimerStep();
				step.setSeqNo(seqNo);
				step.setControllerId(controllerId);
				step.setSubcontrollerId(subId);
				step.setDelay(delay);
				step.setDeviceId(deviceId);
				step.setKeyIndex(keyIndex);
	...
	public static void loadControllers() {
		...
		try {
			while (ctrlSet.next()) {
				...
				Date codeUpdateDT=ctrlSet.getTimestamp("CodeUpdateDT");
				...
				if(null!=codeUpdateDT){
					controller.setCodeUpdateDT(codeUpdateDT.getTime());
				}
		...
		try {
			while (deviceSet.next()) {
				...
				int deviceIndex = deviceSet.getInt("DeviceIndex");
				...
				int circuitCount = deviceSet.getInt("CircuitCount");
		...
		ResultSet IRCodeSet = DB
				.executeQuery("select controllerid, subcontrollerid, deviceid, keyindex, codetype, ircodedata, LastEditDT from mdlearnedircode");
		try {
...
//				}
				...
				int codeType = IRCodeSet.getInt("CodeType");
	...
	public static IRCode getStandardIRCode(int deviceType,
			String brandCode, int IRCodeIndex){
